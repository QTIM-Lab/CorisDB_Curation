...
table_def_orig_csvs_path = "/projects/CorisDB_Curation/Databases/CORISDB_REGISTRY/table_def_orig_csvs"
# table_def_orig_csvs_path = "/projects/CorisDB_Curation/Databases/SOURCE/table_def_orig_csvs"
sample_data_csvs_path = "/projects/CorisDB_Curation/Databases/CORISDB_REGISTRY/sample_data"
tables_for_import_path = "/data/linking_various_systems/coris_db_gbq/tmp_for_import"
...
def create_table_code(table, cols, types):
    template = f"CREATE TABLE IF NOT EXISTS coris_registry.{table} (\n"
    cols_count = len(cols)
    index = 0
    columns_code = ""
...
def create_bulk_insert_statements(table):
    ...
    template = f"\copy coris_registry.{table} FROM '{tables_for_import_path}/{csv}' DELIMITERS ',' NULL AS 'NULL' CSV QUOTE '''' HEADER;\n"
    return template

def create_drop_statements(table):
    # pdb.set_trace()
    template = f"DROP TABLE coris_registry.{table};\n"    
    return template

index_for_deleting_sql_query_files = 0
for csv in [i for i in os.listdir(table_def_orig_csvs_path) if i.find(".csv") != -1 and i.find("scheme_to_csv_mapping.csv") == -1]:
    ...
    if index_for_deleting_sql_query_files == 0:
        os.remove(os.path.join(sample_data_csvs_path, "create_tables.sql"))
        os.remove(os.path.join(sample_data_csvs_path, "bulk_insert_sample_data.sql"))
        os.remove(os.path.join(sample_data_csvs_path, "drop_statements.sql"))
    ...
    index_for_deleting_sql_query_files += 1