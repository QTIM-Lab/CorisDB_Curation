...
def preview(IN, OUT, return_img=False):

    print(IN)
    if IN.find('.dcm') != -1:
        try:
            ds = pydicom.dcmread(os.path.join(IN))
        except:
            pdb.set_trace()
        # Tags
        ...
        if sop_class_desc == "Ophthalmic Photography 8 Bit Image Storage" or sop_class_desc == "Secondary Capture Image Storage":
            ...
            ## image.show()
            image.save(os.path.join(OUT, os.path.splitext(os.path.basename(IN))[0]+".png"))  # To save the image to a file (e.g., PNG format)
        elif sop_class_desc == "Encapsulated PDF Storage":
            output_folder = os.path.join(OUT,"pdf_"+os.path.splitext(os.path.basename(IN))[0])
            pdf_file = os.path.join(output_folder, os.path.splitext(os.path.basename(IN))[0]+".pdf")
            ...
            # with open(os.path.join(OUT,"dcm_header_json"+os.path.splitext(os.path.basename(IN))[0]+".json"), "w") as file:
            #     dicom_header_json = json.loads(ds.to_json())
            #     file.write(json.dumps(dicom_header_json, indent=4))
            # pdb.set_trace()
            with open(os.path.join(OUT,"dcm_print_header_"+os.path.splitext(os.path.basename(IN))[0]+".txt"), "w") as file: file.write(str(ds))
        else:
            # pdb.set_trace()
            ...
            # ds.pixel_array
            with open(os.path.join(OUT,"dcm_print_header_"+os.path.splitext(os.path.basename(IN))[0]+".txt"), "w") as file: file.write(str(ds))
    elif IN.find('.j2k') != -1: # J2K
        try:
            # pdb.set_trace()
            image = Image.open(os.path.join(IN, IN))
        except:
            print(f"missing {IN}")
            # pdb.set_trace()
            pass
        ## pdb.set_trace()
        ## row = all_devices_1_each[all_devices_1_each['IN_coris'] == IN]
