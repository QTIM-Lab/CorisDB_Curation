...
select distinct devname from axispacs_snowflake.devices

select * from axispacs_snowflake.devices where devname like '%NonMyd%'
...
from axispacs_snowflake.file_paths_and_meta as f inner join glaucoma.fundus_encounters as fe
    ...
    and fe.Day = f.Day
where f.devname in ('Nidek') and f.fileeye ='OS'
...
where f.devname in ('Nidek') and f.fileeye in ('OS', 'OD')
-- limit 4 --and f.ptid = ''
