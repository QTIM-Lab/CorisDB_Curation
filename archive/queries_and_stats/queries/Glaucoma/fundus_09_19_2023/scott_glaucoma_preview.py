import os, pandas as pd, pydicom, pdb, numpy, json, shutil
from PIL import Image, ImageDraw, ImageFont
from PyPDF2 import PdfReader

os.chdir("/projects/coris_db/postgres/queries_and_stats/queries/Glaucoma/fundus_09_19_2023")
os.getcwd()
...
IN="/data/FUNDUS_Scott_09_21_2023"
# OUT="/data/FUNDUS_Scott_09_21_2023"
OUT="/data/scott_tmp_fundus_09212023"

# samples = pd.read_csv("12_samples.csv")
samples = pd.read_csv("nidek_os.csv")
print(samples.columns)
def copy_images(row):
    # pdb.set_trace()
    print(row['file_path_coris'])
    
    print(row['path_for_scott'])
    os.makedirs(os.path.dirname(row['path_for_scott']), exist_ok=True)
    shutil.copy(row['file_path_coris'], row['path_for_scott'])


samples[['file_path_coris','path_for_scott']].apply(copy_images, axis=1)


file_names = samples['path_for_scott']
for i in range(len(file_names)):
    print(file_names[i])
print(i)

def preview(file_names):
    ...
    # for file_path in file_names_###:
        ...
        if file_path.find('.dcm') != -1:
            ...
            # output_path = os.path.join(OUT, os.path.splitext(os.path.basename(file_path))[0]+".png")
            # print(os.path.split(file_path)[0].split(os.path.sep)[1]) # visit id
            output_path = os.path.join(OUT, os.path.splitext(os.path.basename(file_path))[0]+".png")
            print(output_path)
            image.save(os.path.join(OUT, os.path.splitext(os.path.basename(file_path))[0]+".png"))  # To save the image to a file (e.g., PNG format)
