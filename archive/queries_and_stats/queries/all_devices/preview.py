...
OUT="/projects/coris_db/postgres/queries_and_stats/queries/all_devices/preview/out"
...
# FUNC Start
# dcm_record = all_devices_1_each.iloc[0]

file_names = list(all_devices_1_each['file_path_coris'].apply(lambda x: os.path.basename(x)))

# DICOM
for file_path in all_devices_1_each['file_path_coris'][69:]:
    print(file_path)
    if file_path.find('.dcm') != -1:
        try:
            ds = pydicom.dcmread(os.path.join("raw", file_path))
        except:
            pdb.set_trace()
        # Tags
