...
os.chdir("/projects/coris_db/postgres/queries_and_stats/queries/all_devices/")
os.getcwd()
from pdf_to_png import pdf_to_png

IN="/projects/coris_db/postgres/queries_and_stats/queries/all_devices/HVFs/preview/raw"
OUT="/projects/coris_db/postgres/queries_and_stats/queries/all_devices/HVFs/preview/out"
FONT_DIR="/projects/coris_db/postgres/queries_and_stats/queries/all_devices/Roboto/Roboto-Black.ttf"

file_names = list(os.listdir(IN))
...
for file_path in file_names:
    ...
    if file_path.find('.dcm') != -1:
        ...
        elif sop_class_desc == "Ophthalmic Visual Field Static Perimetry Measurements Storage":
            with open(os.path.join(OUT,"dcm_print_header_"+os.path.splitext(os.path.basename(file_path))[0]+".txt"), "w") as file: file.write(str(ds))
        else:
            pdb.set_trace()
        ...
        try:
            image = Image.open(os.path.join("raw",file_path))
            # image = Image.open(os.path.join("raw","1.2.276.0.75.2.2.45.215507186569430.20210802140828154.7880372920-1.j2k"))
        except:
