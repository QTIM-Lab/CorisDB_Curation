...
CREATE UNLOGGED TABLE encYMD AS         --getting enc date_key, arb_person_id, arb_encounter_id
    ...
    join cohortAMD on cohortAMD.arb_person_id=enc.arb_person_id
;       
CREATE INDEX idx_encYMD_person_date ON encYMD (arb_encounter_id, date_key);

CREATE UNLOGGED TABLE temp_note AS     --getting temp_note based on arb_encounter_id (filtered by cohortAMD)
